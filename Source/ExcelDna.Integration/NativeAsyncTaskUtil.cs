...
// Helpers for creating and using Task-based functions with Excel 2010+ native
// async support
public static class NativeAsyncTaskUtil {
  ...
  // CONSIDER: Is it important to call this, or will Excel / Excel-DNA clean up
  // when if the add-in gets unloaded?
  ...
  // We keep a CancellationTokenSource around, and set to a new one whenever a
  // calculation has finished.
  static CancellationTokenSource _cancellation = new CancellationTokenSource();
  static void CalculationCanceled() { _cancellation.Cancel(); }

  static void CalculationEnded() {
    // Set to a fresh CancellationTokenSource after every calculation is
    // complete
  ...
  // but keep the Func<Task<T>> indirection to be consistent with the similar
  // calls in AsyncTaskUtil.
  public static void RunTask<TResult>(Func<Task<TResult>> taskSource,
  ...
  public static void RunTaskWithCancellation<TResult>(
  ...
  public static void RunAsTask<TResult>(Func<TResult> function,
                                        ExcelAsyncHandle asyncHandle) {
  ...
  public static void
  RunAsTaskWithCancellation<TResult>(Func<CancellationToken, TResult> function,
                                     ExcelAsyncHandle asyncHandle) {
    CancellationToken cancellationToken = _cancellation.Token;
    var task = Task.Factory.StartNew(() => function(cancellationToken),
                                     cancellationToken);
    task.ContinueWith(t => {
      try {
        // task.Result will throw an AggregateException if there was an error
