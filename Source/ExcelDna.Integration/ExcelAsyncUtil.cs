...
// Task.ToObservable:
// http://blogs.msdn.com/b/pfxteam/archive/2010/04/04/9990349.aspx

// Pattern for making an Observable:
// http://msdn.microsoft.com/en-us/library/dd990377.aspx Task.ToObservable:
// http://blogs.msdn.com/b/pfxteam/archive/2010/04/04/9990349.aspx
...
// The next two interfaces would be regular System.IObservable<object> if we
// could target .NET 4.
// TODO: Make an adapter to make it easy to integrate with .NET 4.
public interface IExcelObservable {
  [NotNull]
  [MustUseReturnValue] // ReSharper will warn developer to use return value,
                       // e.g. to dispose channel
...
public static class ExcelAsyncUtil {
  [Obsolete(
      "ExcelAsyncUtil.Initialize is no longer required. The call can be removed.")]
  public static void Initialize() {}
  [Obsolete(
      "ExcelAsyncUtil.Uninitialize is no longer required. The call can be removed.")]
  public static void Uninitialize() {}

  // Async observable support
  // This is the most general RTD registration
  // ThreadSafe
  public static object Observe(string callerFunctionName,
                               object callerParameters,
                               ExcelObservableSource observableSource) {
    return Observe(callerFunctionName, callerParameters,
                   ExcelObservableOptions.None, observableSource);
  }
  ...
  public static object Observe(string callerFunctionName,
                               ...
                               ExcelObservableSource observableSource) {
    return AsyncObservableImpl.ProcessObservable(
        callerFunctionName, callerParameters, options, observableSource);
  }

  public static object Observe<T>(string callerFunctionName,
                                  object callerParameters,
  ...
  // ThreadSafe
  public static object Run(string callerFunctionName, object callerParameters,
                           ExcelFunc asyncFunc) {
    Debug.Print("ExcelAsyncUtil.Run - {0} : {1}", callerFunctionName,
                callerParameters);
    return AsyncObservableImpl.ProcessFunc(callerFunctionName, callerParameters,
                                           asyncFunc);
  }
  ...
  // ThreadSafe
  public static object Run(string callerFunctionName, object callerParameters,
                           ExcelFuncAsyncHandle asyncFunc) {
    return AsyncObservableImpl.ProcessFuncAsyncHandle(
        callerFunctionName, callerParameters, asyncFunc);
  }

  public static object RunTask<TResult>(string callerFunctionName,
                                        object callerParameters,
  ...
  // Careful - this might only work as long as the task is not shared between
  // calls, since cancellation cancels that task
  public static object RunTaskWithCancellation<TResult>(
      string callerFunctionName, object callerParameters,
  ...
  public static object RunAsTask<TResult>(string callerFunctionName,
                                          object callerParameters,
  ...
  public static object RunAsTaskWithCancellation<TResult>(
      string callerFunctionName, object callerParameters,
      Func<CancellationToken, TResult> function) {
    return RunTaskWithCancellation(
        callerFunctionName, callerParameters,
  ...
  public static void QueueAsMacro(SendOrPostCallback callback, object state) {
    SynchronizationManager.RunMacroSynchronization.RunAsMacroAsync(callback,
                                                                   state);
  }
  ...
  public static event ExcelAction CalculationCanceled {
    add {
      if (_calculationCanceled != null) {
        ...
        _calculationCanceled = value;
        double result =
            (double)XlCall.Excel(XlCall.xlEventRegister, "CalculationCanceled",
                                 XlCall.xleventCalculationCanceled);
        if (result == 0) {
    ...
    remove {
      _calculationCanceled -= value;
      if (_calculationCanceled == null) {
        XlCall.Excel(XlCall.xlEventRegister, null,
                     XlCall.xleventCalculationCanceled);
      }
    }
  }
  ...
  public static event ExcelAction CalculationEnded {
    add {
      if (_calculationEnded != null) {
        ...
        _calculationEnded = value;
        double result =
            (double)XlCall.Excel(XlCall.xlEventRegister, "CalculationEnded",
                                 XlCall.xleventCalculationEnded);
        if (result == 0) {
          // CONSIDER: Is there a better way to handle this unexpected error?
          throw new XlCallException(XlCall.XlReturn.XlReturnFailed);
        }
